  ...
  do { /* iteratively increase precision */
	...
	for(i=0; i<n; i++) { /*** example loop ***/
	  ...
	  if(opti[i] != opti_round) {/* if the example is not shrunk
	    ...
	    if(0 && slack > (MAX(margin-dist,0.0)+0.0001)) {
  ...
  */
  ...
  do { /* iteratively find and add constraints to working set */
	...
	for(i=0; i<n; i++) {
	  ...
	  append_svector_list(fybar,lhs);  /* add fy-fybar to lhs */
	  lhs=fybar;
      ...
      /**** store vector list more compactly in form of the sum ****/
      if(kparm->kernel_type == LINEAR) {
	diff=add_list_ns(lhs); /* compute weighted sum */
	free_svector(lhs);
	lhs=diff;
      }
      ...
      if(0 && slack > (MAX(margin-dist,0.0)+0.000001)) {
	printf("\nWARNING: Slack of most violated constraint is smaller than slack of working\n");
	printf("         set! There is probably a bug in 'find_most_violated_constraint_*'.\n");
	...
	if(sparm->slack_norm == 1) 
	  cset.lhs[cset.m]=create_example(cset.m,0,1,1,copy_svector(lhs));
	else if(sparm->slack_norm == 2)
	  exit(1);
	...
	printf("(NumConst=%d, SV=%ld, CEps=%.4f, QPEps=%.4f)\n",cset.m,
      ...
      free_example(doc,0);
      free_svector(lhs); /* this free's all fy and fybar */
	...
	that update_constraint_cache_for_model has been run */
{
  ...
  for(i=0; i<ccache->n; i++) { 
    fydelta=copy_svector(ccache->constlist[i]->fydelta);
